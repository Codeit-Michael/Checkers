	...
	def get_possible_moves(self, board):
		...
		for i in range(1, 8):
			if self.x + i > 7 or self.y - i < 0:
				break
			moves_ne.append(board.get_tile_from_pos(
				(self.x + i, self.y - i)
			))
		...
		for i in range(1, 8):
			if self.x + i > 7 or self.y + i > 7:
				break
			moves_se.append(board.get_tile_from_pos(
				(self.x + i, self.y + i)
			))
		...
		for i in range(1, 8):
			if self.x - i < 0 or self.y + i > 7:
				break
			moves_sw.append(board.get_tile_from_pos(
				(self.x - i, self.y + i)
			))
		...
		for i in range(1, 8):
			if self.x - i < 0 or self.y - i < 0:
				break
			moves_nw.append(board.get_tile_from_pos(
				(self.x - i, self.y - i)
			))
